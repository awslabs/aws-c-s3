...
/**
 * Initialize a bitmap of the given capacity and set every 1-based index from the array.
 * Raises AWS_ERROR_INVALID_ARGUMENT (and cleans up the bitmap) if any index is 0 or exceeds capacity.
 */
AWS_STATIC_IMPL int aws_s3_bitmap_init_from_array(
    struct aws_byte_buf *bitmap,
    struct aws_allocator *allocator,
    uint32_t capacity,
    ...
    if (aws_s3_bitmap_init(bitmap, allocator, capacity)) {
        return AWS_OP_ERR;
    }
    for (size_t i = 0; i < num_indices; ++i) {
        if (indices[i] == 0 || indices[i] > capacity) {
            aws_byte_buf_clean_up(bitmap);
            return aws_raise_error(AWS_ERROR_INVALID_ARGUMENT);
        }
        aws_s3_bitmap_set(bitmap, indices[i]);
    }
    return AWS_OP_SUCCESS;
}

/**
 * Append every set 1-based index (up to capacity) to out_indices.
 * out_indices must be a caller-initialized aws_array_list of uint32_t.
 */
AWS_STATIC_IMPL int aws_s3_bitmap_to_array(
    const struct aws_byte_buf *bitmap,
    uint32_t capacity,
    struct aws_array_list *out_indices) {

    for (uint32_t i = 1; i <= capacity; ++i) {
        if (aws_s3_bitmap_get(bitmap, i)) {
            if (aws_array_list_push_back(out_indices, &i)) {
                return AWS_OP_ERR;
            }
        }
    }
