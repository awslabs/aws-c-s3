...
parser.add_argument(
    "action", choices=["init", "clean"], help="Initialize or clean up the test buckets"
)
parser.add_argument(
    "bucket_name",
    nargs="?",
...
def put_pre_existing_objects(
    size, key_name, bucket=BUCKET_NAME, sse=None, public_read=False
):
    if size == 0:
        s3_client.put_object(Bucket=bucket, Key=key_name)
        print(f"Object {key_name} uploaded")
        return

    body = create_bytes(size)
    args = {"Bucket": bucket, "Key": key_name, "Body": body}
    if sse == "aes256":
    ...
    try:
        ...
        print(f"Object {key_name} failed to upload, with exception: {e}")
        if public_read and e.response["Error"]["Code"] == "AccessDenied":
            print(
                f"Check your account level S3 settings, public access may be blocked."
            )
        exit(-1)
    print(f"Object {key_name} uploaded")
...
def create_presigned_url(bucket_name, object_name, expiration=3600):
    ...
    :param object_name: The object key saved in the bucket.
    :param expiration: The expiration of the url. Default is 3600s.
    ...
    try:
        ...
        print(f"Failed to generate a presigned url with exception {e}")
        return None
    return response
...
def create_bucket_with_lifecycle():
    try:
        ...
        put_pre_existing_objects(10 * MB, "pre-existing-10MB-aes256-c", sse="aes256-c")
        put_pre_existing_objects(10 * MB, "pre-existing-10MB-aes256", sse="aes256")
        put_pre_existing_objects(10 * MB, "pre-existing-10MB-kms", sse="kms")
...
def create_bucket_with_public_object():
    try:
        ...
        if (
            ...
            print(
                f"Bucket {PUBLIC_BUCKET_NAME} not created, skip initializing.",
                file=sys.stderr,
            )
            return
        raise e
...
if args.action == "init":
    ...
    if "CRT_S3_TEST_BUCKET_NAME" not in os.environ and args.bucket_name is None:
        print(
            "Set the environment variable CRT_S3_TEST_BUCKET_NAME before clean up, or pass in bucket_name as argument."
        )
        exit(-1)
    cleanup(BUCKET_NAME)
    cleanup(PUBLIC_BUCKET_NAME)
