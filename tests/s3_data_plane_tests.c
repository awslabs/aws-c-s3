...
static int s_test_s3_upload_part_message_helper(struct aws_allocator *allocator, bool should_compute_content_md5) {
    ...
    struct aws_http_message *new_message = aws_s3_upload_part_message_new(
        allocator, base_message, &test_buffer, part_number, upload_id, should_compute_content_md5, NULL);
...
static int s_check_metrics_helper(
    struct aws_s3_meta_request_test_results *test_results,
    size_t index,
    enum aws_s3_request_type expected_type,
    ...
    aws_s3_request_metrics_get_part_range_start(metrics, &range_start);
    aws_s3_request_metrics_get_part_range_end(metrics, &range_end);
    ASSERT_UINT_EQUALS(expected_type, out_request_type);
    ASSERT_UINT_EQUALS(expected_part_number, part_number);
    ASSERT_UINT_EQUALS(expected_range_start, range_start);
    ASSERT_UINT_EQUALS(expected_range_end, range_end);
...
static int s_test_s3_round_trip_dynamic_range_size_download_multipart(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    struct aws_s3_tester tester;
    ASSERT_SUCCESS(aws_s3_tester_init(allocator, &tester));
...

    {
        ...
        ASSERT_SUCCESS(aws_s3_tester_upload_file_path_init(
            allocator, &path_buf, aws_byte_cursor_from_c_str("/prefix/round_trip/dynamic_size_multipart")));
        struct aws_byte_cursor object_path = aws_byte_cursor_from_buf(&path_buf);
        /* Configure file I/O options for streaming upload */
        struct aws_s3_file_io_options fio_opts = {
            .should_stream = true,
        ...
        /*** GET FILE WITHOUT CHECKSUM -- RANGE get first ***/
        aws_s3_meta_request_test_results_init(&test_results, allocator);

        struct aws_s3_tester_meta_request_options get_options_without_checksum = {
            .allocator = allocator,
            .meta_request_type = AWS_S3_META_REQUEST_TYPE_GET_OBJECT,
            .validate_type = AWS_S3_TESTER_VALIDATE_TYPE_EXPECT_SUCCESS,
            .client = client,
...
static int s_test_s3_round_trip_dynamic_range_size_download_single_part(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    struct aws_s3_tester tester;
    ASSERT_SUCCESS(aws_s3_tester_init(allocator, &tester));
...
static int s_test_s3_bad_endpoint(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_TRUE(
        ...
        meta_request_test_results.finished_error_code == AWS_IO_DNS_QUERY_AGAIN ||
...
AWS_TEST_CASE(test_s3_empty_file_edge_case, s_test_s3_empty_file_edge_case)
static int s_test_s3_empty_file_edge_case(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    ...
    struct aws_s3_meta_request_test_results results;
    {
        /***** Empty download failed with range *****/
        aws_s3_meta_request_test_results_init(&results, allocator);
        struct aws_s3_tester_meta_request_options options = {
            .allocator = allocator,
            .client = client,
    ...
    {
        /***** Empty download succeed with force_dynamic_part_size *****/
        aws_s3_meta_request_test_results_init(&results, allocator);
        struct aws_s3_tester_meta_request_options force_dynamic_options = {
            .allocator = allocator,
            .client = client,
            .meta_request_type = AWS_S3_META_REQUEST_TYPE_GET_OBJECT,
            .validate_type = AWS_S3_TESTER_VALIDATE_TYPE_EXPECT_SUCCESS,
        ...
        ASSERT_SUCCESS(aws_s3_tester_send_meta_request_with_options(&tester, &force_dynamic_options, &results));
...
/*******************************************************************************
 * Async pause/resume tests
 ******************************************************************************/

/* Shared test state for async pause tests */
struct async_pause_resume_test_data {
    struct aws_mutex mutex;
    struct aws_condition_variable c_var;
    ...
    /* Atomics for the patched finished_request */
    struct aws_atomic_var parts_completed;
    struct aws_atomic_var pause_after_n_parts;
    struct aws_atomic_var pause_initiated;
};

static void s_async_pause_meta_request_finish(
    struct aws_s3_meta_request *meta_request,
    const struct aws_s3_meta_request_result *meta_request_result,
    void *user_data) {
    (void)meta_request;
...
static bool s_async_pause_completion_predicate(void *arg) {
    struct async_pause_resume_test_data *test_data = arg;
    return test_data->execution_completed;
}
...
static void s_async_pause_complete_callback(
    ...
    void *user_data) {
    (void)meta_request;
    struct async_pause_resume_test_data *test_data = user_data;
...
/* Patched finished_request for PUT async pause: triggers pause_async after N parts */
static void s_put_async_pause_finished_request(
    struct aws_s3_meta_request *meta_request,
    struct aws_s3_request *request,
    int error_code) {
    struct aws_s3_tester *tester = meta_request->user_data;
    ...
    if ((error_code == AWS_ERROR_SUCCESS) && (meta_request->type == AWS_S3_META_REQUEST_TYPE_PUT_OBJECT) &&
        (request->request_tag == AWS_S3_AUTO_RANGED_PUT_REQUEST_TAG_PART) && !request->is_noop) {
...
static void s_get_async_pause_finished_request(
    ...
    if ((error_code == AWS_ERROR_SUCCESS) && (meta_request->type == AWS_S3_META_REQUEST_TYPE_GET_OBJECT) &&
...
static struct aws_s3_meta_request *s_meta_request_factory_put_async_pause(
    struct aws_s3_client *client,
    const struct aws_s3_meta_request_options *options) {
    struct aws_s3_tester *tester = client->shutdown_callback_user_data;
    struct aws_s3_client_vtable *original_client_vtable =
...
static struct aws_s3_meta_request *s_meta_request_factory_get_async_pause(
    struct aws_s3_client *client,
    const struct aws_s3_meta_request_options *options) {
    struct aws_s3_tester *tester = client->shutdown_callback_user_data;
    struct aws_s3_client_vtable *original_client_vtable =
    ...
    struct aws_s3_meta_request_vtable *patched =
        aws_s3_tester_patch_meta_request_vtable(tester, meta_request, NULL);
    patched->finished_request = s_get_async_pause_finished_request;
    return meta_request;
}

static void s_async_pause_test_data_init(struct async_pause_resume_test_data *test_data) {
    AWS_ZERO_STRUCT(*test_data);
    test_data->c_var = (struct aws_condition_variable)AWS_CONDITION_VARIABLE_INIT;
    aws_mutex_init(&test_data->mutex);
    aws_atomic_init_int(&test_data->parts_completed, 0);
    aws_atomic_init_int(&test_data->pause_after_n_parts, 0);
    aws_atomic_init_int(&test_data->pause_initiated, false);
}

static void s_async_pause_test_data_clean_up(struct async_pause_resume_test_data *test_data) {
...
static int s_test_s3_put_pause_resume_async_happy_path(struct aws_allocator *allocator, void *ctx) {
    ...
    /* Verify the token has a valid upload_id */
    struct aws_byte_cursor upload_id = aws_s3_meta_request_resume_token_upload_id(test_data.resume_token);
    ASSERT_TRUE(upload_id.len > 0);

    aws_s3_meta_request_release(meta_request);
    aws_http_message_destroy(message);
    aws_input_stream_destroy(upload_stream);
    client = aws_s3_client_release(client);
    aws_s3_tester_wait_for_client_shutdown(&tester);
    tester.bound_to_client = false;
    ...
    aws_atomic_store_int(&resume_data.pause_after_n_parts, SIZE_MAX); /* don't pause on resume */
    ...
    /* Verify the uploaded object by downloading and checking content */
    struct aws_s3_tester_meta_request_options get_options = {
        .allocator = allocator,
        .meta_request_type = AWS_S3_META_REQUEST_TYPE_GET_OBJECT,
        .validate_type = AWS_S3_TESTER_VALIDATE_TYPE_EXPECT_SUCCESS,
    ...
    struct aws_s3_meta_request_test_results get_results;
    aws_s3_meta_request_test_results_init(&get_results, allocator);
    ASSERT_SUCCESS(aws_s3_tester_send_meta_request_with_options(&tester, &get_options, &get_results));
    aws_s3_meta_request_test_results_clean_up(&get_results);
...
static int s_test_s3_get_pause_resume_async_to_file(struct aws_allocator *allocator, void *ctx) {
    ...
    /* Wait for the pause leg to complete */
    aws_mutex_lock(&test_data.mutex);
    aws_condition_variable_wait_pred(&test_data.c_var, &test_data.mutex, s_async_pause_completion_predicate, &test_data);
    aws_mutex_unlock(&test_data.mutex);
    ...
    /* Verify token fields */
    struct aws_byte_cursor etag = aws_s3_meta_request_resume_token_etag(test_data.resume_token);
    ASSERT_TRUE(etag.len > 0);
    ASSERT_TRUE(aws_s3_meta_request_resume_token_object_size(test_data.resume_token) == s_get_async_pause_object_size);
    ASSERT_TRUE(aws_s3_meta_request_resume_token_num_parts_completed(test_data.resume_token) > 0);
    ...
    /* Step 3: Resume download to the same file */
    struct async_pause_resume_test_data resume_data;
    s_async_pause_test_data_init(&resume_data);
    aws_atomic_store_int(&resume_data.pause_after_n_parts, SIZE_MAX); /* don't pause on resume */
    tester.user_data = &resume_data;
...
/* ---- Test 9: GET async pause/resume via body callback (no file) ---- */

/* Combined test data for body callback tests -- includes both pause state and body tracking */
struct get_async_body_callback_combined_data {
    struct async_pause_resume_test_data pause_data;
    struct aws_byte_buf received_body;
    struct aws_mutex body_mutex;
    uint64_t total_bytes_received;
};

static int s_get_async_body_callback(
    struct aws_s3_meta_request *meta_request,
    const struct aws_byte_cursor *body,
    uint64_t range_start,
    void *user_data) {
    ...
    (void)range_start;
    struct aws_s3_tester *tester = user_data;
    struct get_async_body_callback_combined_data *combined = tester->user_data;
    aws_mutex_lock(&combined->body_mutex);
    aws_byte_buf_append_dynamic(&combined->received_body, body);
    combined->total_bytes_received += body->len;
    aws_mutex_unlock(&combined->body_mutex);
    return AWS_OP_SUCCESS;
}

AWS_TEST_CASE(test_s3_get_pause_resume_async_body_callback, s_test_s3_get_pause_resume_async_body_callback)
static int s_test_s3_get_pause_resume_async_body_callback(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    struct aws_s3_tester tester;
    AWS_ZERO_STRUCT(tester);
    ASSERT_SUCCESS(aws_s3_tester_init(allocator, &tester));
    ...
    aws_mutex_lock(&pause_combined.pause_data.mutex);
    aws_condition_variable_wait_pred(
        &pause_combined.pause_data.c_var,
        &pause_combined.pause_data.mutex,
        s_async_pause_completion_predicate,
        &pause_combined.pause_data);
    aws_mutex_unlock(&pause_combined.pause_data.mutex);

    ASSERT_INT_EQUALS(AWS_ERROR_S3_PAUSED, pause_combined.pause_data.meta_request_error_code);
    ASSERT_NOT_NULL(pause_combined.pause_data.resume_token);

    /* Record how many bytes we got in the pause leg */
    uint64_t pause_leg_bytes = pause_combined.total_bytes_received;
    ASSERT_TRUE(pause_leg_bytes > 0);
    ASSERT_TRUE(pause_leg_bytes < s_get_async_pause_object_size);

    /* Verify token fields (callback delivery, no file mtime) */
    ASSERT_TRUE(
        aws_s3_meta_request_resume_token_object_size(pause_combined.pause_data.resume_token) ==
        s_get_async_pause_object_size);
    ...
    /* --- Resume leg: download remaining parts via body callback --- */
    struct get_async_body_callback_combined_data resume_combined;
    ...
    struct aws_s3_meta_request_options resume_get_options = {
        ...
        .message = message,
        .type = AWS_S3_META_REQUEST_TYPE_GET_OBJECT,
        .resume_token = pause_combined.pause_data.resume_token,
    };

    meta_request = aws_s3_client_make_meta_request(client, &resume_get_options);
    ASSERT_NOT_NULL(meta_request);

    aws_mutex_lock(&resume_combined.pause_data.mutex);
    aws_condition_variable_wait_pred(
        &resume_combined.pause_data.c_var,
        &resume_combined.pause_data.mutex,
        s_async_pause_completion_predicate,
        &resume_combined.pause_data);
    aws_mutex_unlock(&resume_combined.pause_data.mutex);

    ASSERT_INT_EQUALS(AWS_ERROR_SUCCESS, resume_combined.pause_data.meta_request_error_code);

    /* Resume leg should deliver fewer bytes than full download (only missing parts) */
    uint64_t resume_leg_bytes = resume_combined.total_bytes_received;
    ASSERT_TRUE(resume_leg_bytes > 0);
    ASSERT_TRUE(resume_leg_bytes < s_get_async_pause_object_size);

    /* Combined coverage: pause_leg + resume_leg = full object */
    ASSERT_UINT_EQUALS(s_get_async_pause_object_size, pause_leg_bytes + resume_leg_bytes);
    ...
    aws_byte_buf_clean_up(&pause_combined.received_body);
    aws_mutex_clean_up(&pause_combined.body_mutex);
    aws_byte_buf_clean_up(&resume_combined.received_body);
    aws_mutex_clean_up(&resume_combined.body_mutex);
    s_async_pause_test_data_clean_up(&pause_combined.pause_data);
    s_async_pause_test_data_clean_up(&resume_combined.pause_data);
    aws_s3_tester_clean_up(&tester);
    return AWS_OP_SUCCESS;
}

/* ---- Test 10: GET double pause_async returns INVALID_STATE ---- */

AWS_TEST_CASE(test_s3_get_pause_async_twice, s_test_s3_get_pause_async_twice)
static int s_test_s3_get_pause_async_twice(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    struct aws_s3_tester tester;
    AWS_ZERO_STRUCT(tester);
    ASSERT_SUCCESS(aws_s3_tester_init(allocator, &tester));
    ...
    /* Second pause callback state */
    struct async_pause_resume_test_data second_pause_data;
    s_async_pause_test_data_init(&second_pause_data);
    ...
    struct aws_s3_meta_request_options get_options = {
        ...
        .message = message,
        .type = AWS_S3_META_REQUEST_TYPE_GET_OBJECT,
        .recv_filepath = aws_byte_cursor_from_c_str("test_get_pause_async_twice_tmp"),
    };
    ...
    /* Now call pause_async again on the same meta_request -- should get INVALID_STATE in callback */
    int second_result = aws_s3_meta_request_pause_async(meta_request, s_async_pause_complete_callback, &second_pause_data);
    /* pause_async returns success (it invokes the callback with the error, doesn't fail the call itself) */
    ASSERT_INT_EQUALS(AWS_OP_SUCCESS, second_result);
    ASSERT_TRUE(second_pause_data.pause_callback_invoked);
    ASSERT_INT_EQUALS(AWS_ERROR_INVALID_STATE, second_pause_data.pause_error_code);
    ASSERT_NULL(second_pause_data.resume_token);
    ...
    remove("test_get_pause_async_twice_tmp");
    s_async_pause_test_data_clean_up(&test_data);
    s_async_pause_test_data_clean_up(&second_pause_data);
    aws_s3_tester_clean_up(&tester);
    return AWS_OP_SUCCESS;
}

/* ---- Test 11: GET resume after file modification -> silent restart, full re-download ---- */

AWS_TEST_CASE(test_s3_get_resume_file_modified_restart_e2e, s_test_s3_get_resume_file_modified_restart_e2e)
static int s_test_s3_get_resume_file_modified_restart_e2e(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    struct aws_s3_tester tester;
    AWS_ZERO_STRUCT(tester);
    ASSERT_SUCCESS(aws_s3_tester_init(allocator, &tester));
    ...
    /* --- Mutate the file: append a byte to invalidate its mtime --- */
    FILE *mutate_file = aws_fopen(download_filepath, "ab");
    ASSERT_NOT_NULL(mutate_file);
    fputc('X', mutate_file);
    fclose(mutate_file);

    /* --- Resume leg: should detect mtime mismatch and do full re-download --- */
    struct async_pause_resume_test_data resume_data;
    s_async_pause_test_data_init(&resume_data);
    aws_atomic_store_int(&resume_data.pause_after_n_parts, SIZE_MAX);
    tester.user_data = &resume_data;
    ...
    /* Should succeed despite file modification -- silently restarts */
    ASSERT_INT_EQUALS(AWS_ERROR_SUCCESS, resume_data.meta_request_error_code);
    ASSERT_INT_EQUALS(AWS_HTTP_STATUS_CODE_200_OK, resume_data.response_status_code);
