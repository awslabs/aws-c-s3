...
/* Test that an MPU can be done with checksum location = NONE as long as an upload review callback
 * is used, and the resulting object doesn't have checksums uploaded. */
AWS_TEST_CASE(test_s3_upload_review_checksum_location_none_crc32c, s_test_s3_upload_review_checksum_location_none_crc32c)
static int s_test_s3_upload_review_checksum_location_none_crc32c(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;

    struct aws_byte_cursor object_path = aws_byte_cursor_from_c_str("/upload/review_1024MB_no_CRC32C.txt");
