...
static void s_s3_test_meta_request_finish(
    ...
    /* Lane state is auto-ranged-GET-only. The type check is necessary: a GET whose query has
    ...
    if (meta_request->type == AWS_S3_META_REQUEST_TYPE_GET_OBJECT && meta_request->part_size != 0) {
        struct aws_s3_auto_ranged_get *auto_ranged_get = meta_request->impl;
        meta_request_test_results->spread_count = auto_ranged_get->synced_data.spread_count;
        /* No lanes_remaining: block-cyclic spread has no per-lane state to check. */
    }
    aws_s3_meta_request_unlock_synced_data(meta_request);
