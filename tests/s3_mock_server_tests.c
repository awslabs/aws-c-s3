...
TEST_CASE(bad_tls_negotiation_mock_server) {
    (void)ctx;
    ...
    struct aws_byte_cursor url_cursor = aws_byte_cursor_from_c_str("https://localhost:8080/");

    struct aws_uri endpoint;
    ASSERT_SUCCESS(aws_uri_init_parse(&endpoint, allocator, &url_cursor));
    struct aws_http_message *message =
        aws_s3_test_get_object_request_new(allocator, *aws_uri_authority(&endpoint), g_pre_existing_object_1MB);
    ...
    ASSERT_UINT_EQUALS(AWS_IO_TLS_ERROR_NEGOTIATION_FAILURE, meta_request_test_results.finished_error_code);
