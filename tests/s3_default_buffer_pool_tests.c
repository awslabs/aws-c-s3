...
static void s_threaded_alloc_worker(void *user_data) {
    ...
    for (size_t count = 0; count < NUM_TEST_ALLOCS / NUM_TEST_THREADS; ++count) {
        ...
        struct aws_s3_buffer_ticket *ticket = aws_s3_default_buffer_pool_reserve(pool, size);
    ...
    for (size_t count = 0; count < NUM_TEST_ALLOCS / NUM_TEST_THREADS; ++count) {
        aws_s3_default_buffer_pool_release_ticket(pool, tickets[count]);
    }
}
...
static int s_test_s3_buffer_pool_limits(struct aws_allocator *allocator, void *ctx) {
    ...
    struct aws_s3_buffer_ticket *ticket1 = aws_s3_default_buffer_pool_reserve(buffer_pool, MB_TO_BYTES(64));
    ...
    for (size_t i = 0; i < 6; ++i) {
        tickets[i] = aws_s3_default_buffer_pool_reserve(buffer_pool, MB_TO_BYTES(128));
    ...
    ASSERT_NULL(aws_s3_default_buffer_pool_reserve(buffer_pool, MB_TO_BYTES(128)));
    ASSERT_NULL(aws_s3_default_buffer_pool_reserve(buffer_pool, MB_TO_BYTES(96)));

    aws_s3_default_buffer_pool_remove_reservation_hold(buffer_pool);
    struct aws_s3_buffer_ticket *ticket2 = aws_s3_default_buffer_pool_reserve(buffer_pool, MB_TO_BYTES(32));
    ...
    for (size_t i = 0; i < 6; ++i) {
        aws_s3_default_buffer_pool_release_ticket(buffer_pool, tickets[i]);
    }

    aws_s3_default_buffer_pool_release_ticket(buffer_pool, ticket1);
    aws_s3_default_buffer_pool_release_ticket(buffer_pool, ticket2);
...
static int s_test_s3_buffer_pool_trim(struct aws_allocator *allocator, void *ctx) {
    ...
    for (size_t i = 0; i < 40; ++i) {
        tickets[i] = aws_s3_default_buffer_pool_reserve(buffer_pool, MB_TO_BYTES(8));
        ASSERT_NOT_NULL(tickets[i]);
        struct aws_byte_buf buf = aws_s3_default_buffer_pool_acquire_buffer(buffer_pool, tickets[i]);
        ASSERT_NOT_NULL(buf.buffer);
    }
...
static int s_test_s3_buffer_pool_reservation_hold(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_NULL(aws_s3_default_buffer_pool_reserve(buffer_pool, MB_TO_BYTES(8)));

    ASSERT_TRUE(aws_s3_default_buffer_pool_has_reservation_hold(buffer_pool));
    ...
    struct aws_s3_buffer_ticket *ticket = aws_s3_default_buffer_pool_reserve(buffer_pool, MB_TO_BYTES(8));
    ASSERT_NOT_NULL(ticket);

    aws_s3_default_buffer_pool_release_ticket(buffer_pool, ticket);
...
static int s_test_s3_buffer_pool_forced_buffer(struct aws_allocator *allocator, void *ctx) {
    ...
    { /* Acquire forced buffer from primary storage */
        ...
        struct aws_byte_buf forced_buf =
            aws_s3_default_buffer_pool_acquire_forced_buffer(buffer_pool, acquire_size, &forced_ticket);
        ...
        aws_s3_default_buffer_pool_release_ticket(buffer_pool, forced_ticket);
    }
...
static int s_test_s3_buffer_pool_forced_buffer_after_reservation_hold(struct aws_allocator *allocator, void *ctx) {
    ...
    aws_array_list_init_dynamic(&normal_tickets, allocator, 1, sizeof(struct aws_s3_buffer_ticket *));
    while (aws_s3_default_buffer_pool_has_reservation_hold(buffer_pool) == false) {
        struct aws_s3_buffer_ticket *normal_ticket = aws_s3_default_buffer_pool_reserve(buffer_pool, chunk_size);
    ...
    struct aws_byte_buf forced_buf_1 =
        aws_s3_default_buffer_pool_acquire_forced_buffer(buffer_pool, chunk_size, &forced_ticket_1);
    ...
    for (size_t i = 0; i < aws_array_list_length(&normal_tickets); ++i) {
        struct aws_s3_buffer_ticket *normal_ticket;
    ...
    struct aws_byte_buf forced_buf_2 =
        aws_s3_default_buffer_pool_acquire_forced_buffer(buffer_pool, chunk_size, &forced_ticket_2);
    ...
    for (size_t i = 0; i < aws_array_list_length(&normal_tickets); ++i) {
        struct aws_s3_buffer_ticket *normal_ticket;
        aws_array_list_get_at(&normal_tickets, &normal_ticket, i);
        aws_s3_default_buffer_pool_release_ticket(buffer_pool, normal_ticket);
    }
    aws_array_list_clean_up(&normal_tickets);

    aws_s3_default_buffer_pool_release_ticket(buffer_pool, forced_ticket_1);
    aws_s3_default_buffer_pool_release_ticket(buffer_pool, forced_ticket_2);
    aws_s3_default_buffer_pool_destroy(buffer_pool);
    return 0;
}
AWS_TEST_CASE(
...
static int s_test_s3_buffer_pool_forced_buffer_wont_stop_reservations(struct aws_allocator *allocator, void *ctx) {
    ...
    struct aws_s3_buffer_ticket *forced_ticket = NULL;
    struct aws_byte_buf forced_buf =
        aws_s3_default_buffer_pool_acquire_forced_buffer(buffer_pool, mem_limit, &forced_ticket);
    ASSERT_NOT_NULL(forced_ticket);
    ...
    struct aws_byte_buf normal_buffer = aws_s3_default_buffer_pool_acquire_buffer(buffer_pool, normal_ticket);
