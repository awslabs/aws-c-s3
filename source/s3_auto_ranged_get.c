...
static int s_s3_auto_ranged_get_init_from_resume_token(
    ...
    auto_ranged_get->synced_data.object_size = resume_token->object_size;
...
struct aws_s3_meta_request *aws_s3_meta_request_auto_ranged_get_new(
    ...
    /* If resuming from a token, restore the bitmap of completed parts so the update loop skips them. */
...
static bool s_s3_auto_ranged_get_update(
    ...
    /* BEGIN CRITICAL SECTION */
    {
        ...
        if (!aws_s3_meta_request_has_finish_result_synced(meta_request)) {
             ...
             * Parts already delivered by a paused request (restored from a resume token) are never
             * re-requested, so they don't produce delivery events in this meta request. */
            uint32_t num_parts_pre_delivered =
                auto_ranged_get->resume_token != NULL ? (uint32_t)auto_ranged_get->resume_token->num_parts_completed
                                                      : 0;
            if (meta_request->synced_data.num_parts_delivery_completed + num_parts_pre_delivered <
...
static struct aws_s3_meta_request_resume_token *s_build_download_resume_token_synced(
    ...
    /* Build the completed-parts bitmap from parts actually delivered to the caller: parts restored
     * from the resume token this request started with, plus the first num_parts_delivery_succeeded
     * parts this request streamed (delivery is strictly in-order, filling the non-restored part
     * numbers in ascending order). Parts whose network request finished but whose body was never
     * streamed to the caller must NOT be marked, or their bytes would be lost on resume. */
    aws_s3_bitmap_init(
        &token->completed_parts_bitmap, meta_request->allocator, auto_ranged_get->synced_data.total_num_parts);
    uint32_t num_streamed_remaining = meta_request->synced_data.num_parts_delivery_succeeded;
    size_t num_parts_delivered = 0;
    for (uint32_t part = 1; part <= auto_ranged_get->synced_data.total_num_parts; ++part) {
        if (aws_s3_bitmap_get(&meta_request->resume_delivered_parts_bitmap, part)) {
            aws_s3_bitmap_set(&token->completed_parts_bitmap, part);
            ++num_parts_delivered;
        } else if (num_streamed_remaining > 0) {
            aws_s3_bitmap_set(&token->completed_parts_bitmap, part);
            --num_streamed_remaining;
            ++num_parts_delivered;
        }
    }
    token->num_parts_completed = num_parts_delivered;
