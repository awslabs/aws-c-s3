...
static void s_s3_auto_ranged_get_request_finished(
    ...
    if (request->discovers_object_size) {
        ...
        if (meta_request->is_express &&
             ...
             * 1. If the Part Size we set is larger than the possible size to hit the limitation, we are safe to make as
             * many connections as we want.
             * 2. If the object size is less than the threshold, we keep our previous behavior, as it's less likely to
             * hit the server side limitation.
             *
