...
struct aws_s3_client *aws_s3_client_new(
    ...
    /* Set up body streaming ELG */
    {
        ...
        client->synced_data.body_streaming_elg_active = true;
    }
...
static void s_s3_client_check_for_shutdown(
    ...
    finish_destroy = client->synced_data.active == false && client->synced_data.allocated_vip_count == 0 &&
                     client->synced_data.host_listener_active == false &&
                     client->synced_data.body_streaming_elg_active == false &&
...
static void s_s3_client_vip_http_connection_manager_shutdown_callback(void *user_data) {
    ...
    AWS_LOGF_INFO(AWS_LS_S3_VIP, "id=%p VIP Connection manager shutdown", (void *)vip);
...
static struct aws_s3_vip *s_s3_client_vip_new(
    ...
    AWS_LOGF_INFO(
        AWS_LS_S3_VIP, "id=%p: Created connection manager %p", (void *)vip, (void *)vip->http_connection_manager);
...
static int s_s3_client_add_vips(struct aws_s3_client *client, const struct aws_array_list *host_addresses) {
    ...
    for (size_t address_index = 0; address_index < aws_array_list_length(host_addresses); ++address_index) {
        ...
        vip = s_s3_client_vip_new(client, &host_address_byte_cursor, &server_name);
...
void aws_s3_client_stream_response_body(
    ...
    AWS_LOGF_DEBUG(AWS_LS_S3_CLIENT, "id=%p Scheduling body streaming task.", (void *)client);
...
static int s_s3_client_start_resolving_addresses_synced(struct aws_s3_client *client) {
    ...
    client->synced_data.host_listener_active = true;
