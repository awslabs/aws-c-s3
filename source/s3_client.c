...
struct aws_s3_client *aws_s3_client_new(
    ...
    aws_atomic_init_int(&client->jitter, 0);
...
static void s_s3_client_on_acquire_http_connection(
    ...
    if (*current_http_connection != incoming_http_connection) {
        ...
        uint32_t max_request_count =
            10 + aws_atomic_fetch_add(&client->jitter, 13) % (s_s3_max_request_count_per_connection - 10);

        *current_http_connection = incoming_http_connection;
        vip_connection->request_count = 0;
        vip_connection->max_request_count = max_request_count;
...
void aws_s3_client_aggregate_throughput(struct aws_s3_client *client, size_t throughput) {
    ...
    aws_atomic_fetch_add(&client->pending_throughput_bytes, throughput);
}
...
static int s_s3_client_add_vips_default(struct aws_s3_client *client, const struct aws_array_list *host_addresses) {
    ...
    if (time_delta > 1.0) {
        size_t pending_throughput_bytes = aws_atomic_exchange_int(&client->pending_throughput_bytes, 0);

        client->synced_data.throughput_timestamp_millis = current_timestamp_millis;

        double rate_in_bytes = (double)pending_throughput_bytes / time_delta;
    ...
    for (size_t address_index = 0; address_index < aws_array_list_length(host_addresses); ++address_index) {
        ...
        if (client->synced_data.active_vip_count >= max_vip_count) {
            break;
        }

        if (current_throughput_gbps >= client->throughput_target_gbps) {
            break;
        }
