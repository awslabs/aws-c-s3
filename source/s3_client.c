...
static void s_s3_client_process_work_default(struct aws_s3_client *client) {
    ...
    s_s3_client_assign_requests_to_connections_threaded(client, AWS_S3_META_REQUEST_UPDATE_FLAG_CONSERVATIVE);
    ...
    s_s3_client_assign_requests_to_connections_threaded(client, 0);
...
static void s_s3_client_assign_requests_to_connections_threaded(
    ...
    aws_linked_list_swap_contents(&vip_connections_updates, &client->threaded_data.connections);
    ...
    while (!aws_linked_list_empty(&vip_connections_updates)) {
        ...
        if (aws_linked_list_empty(&client->threaded_data.meta_requests) ||
            ...
            aws_linked_list_push_back(&client->threaded_data.connections, &vip_connection->node);
            continue;
        }
