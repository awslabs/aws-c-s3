...
struct aws_s3_meta_request *aws_s3_meta_request_auto_ranged_put_new(
    ...
    auto_ranged_put->threaded_prepare_data.next_part_number = 1;
        ...
        case AWS_S3_AUTO_RANGED_PUT_STATE_SENDING_PARTS: {
            ...
            if (auto_ranged_put->synced_data.num_parts_sent < auto_ranged_put->synced_data.total_num_parts) {
                request = aws_s3_request_new(
                    ...
                    AWS_S3_AUTO_RANGED_PUT_REQUEST_TAG_PART,
                    0,
                    AWS_S3_REQUEST_DESC_RECORD_RESPONSE_HEADERS | AWS_S3_REQUEST_DESC_DISPATCH_ASYNC);
    ...
    switch (request->request_tag) {
        ...
        case AWS_S3_AUTO_RANGED_PUT_REQUEST_TAG_PART: {
            if (is_initial_prepare) {
                ...
                request->part_number = auto_ranged_put->threaded_prepare_data.next_part_number;
                ++auto_ranged_put->threaded_prepare_data.next_part_number;
            }
...
static int s_s3_auto_ranged_put_stream_complete(
    ...
    if (request->request_tag == AWS_S3_AUTO_RANGED_PUT_REQUEST_TAG_CREATE_MULTIPART_UPLOAD) {
        ...
        /* set that we are ready for sending parts. */
