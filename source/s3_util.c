...
void aws_s3_request_finish_up_metrics_synced(struct aws_s3_request *request, struct aws_s3_meta_request *meta_request) {
    ...
    if (request->send_data.metrics != NULL) {
        ...
        if (metrics->time_metrics.receive_start_timestamp_ns != -1) {
            metrics->time_metrics.service_call_duration_ns = metrics->time_metrics.receive_start_timestamp_ns - metrics->time_metrics.conn_acquire_start_timestamp_ns;
        }
        ...
        if (metrics->crt_info_metrics.error_code != AWS_S3_CONNECTION_FINISH_CODE_RETRY) {
            ...
            metrics->time_metrics.request_duration_ns = metrics->time_metrics.request_end_timestamp_ns - metrics->time_metrics.request_start_timestamp_ns;
        }
