...
struct aws_s3_request *aws_s3_request_new(
    ...
    request->dispatch_async = (flags & AWS_S3_REQUEST_DESC_DISPATCH_ASYNC) != 0;
...
int aws_s3_meta_request_read_body(struct aws_s3_meta_request *meta_request, struct aws_byte_buf *buffer) {
    ...
    struct aws_input_stream *initial_body_stream = aws_http_message_get_body_stream(meta_request->initial_request_message);
